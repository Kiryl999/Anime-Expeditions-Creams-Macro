...
and not the whole card, so the crop only needs to cover that text with a
little margin. Matching is plain whole-image (no background/transparency
handling) -- crop it as an ordinary rectangular screenshot, background
included.

The label is searched over the WHOLE window, not a fixed band: the card
row renders at two different heights depending on the carousel's state,
about 115px apart, and the old 30px "name band" could only ever see one
of them (see core/stage_select.py). So the label's position on screen
does not matter -- only that the crop is tight and matches the text.
